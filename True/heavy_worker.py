...
def worker_loop(event_queue, result_queue):
    if config.HEAVY_PROCESS_CPU_CORES:
        try:
            ...
            logger.info(f"[WORKER v9.2] CPU affinity {config.HEAVY_PROCESS_CPU_CORES}")
        except Exception as e:
            logger.warning(f"[WORKER v9.2] CPU affinity failed: {e}")

    logger.info("[WORKER v9.2] Запущен и ожидает события...")
    while True:
        try:
        ...
        if snapshot == 'STOP':
            logger.info("[WORKER v9.2] Получен STOP. Завершение.")
            break
...
def process_snapshot(snapshot):
    ...
    p_time_abs = UTCDateTime(t[p_idx])
    ...
    try:
        ...
        if len(h_zone) > 100:
            ...
            triggers_s = trigger_onset(cft_s, 2.2, 1.2)
            if len(triggers_s) > 0:
...
def _analyze_spectrum(data, sample_rate):
    try:
        ...
        freqs, psd = welch(sig, fs=sample_rate, nperseg=1024, noverlap=512)
