...
class AD7606B:
    ...
    def _init_adc(self):
        logger.info(f"[AD7606B {VERSION}] Init...")
        self._reset_adc()
        time.sleep(0.5)
        self._reg_read(0x02)
        self._configure_adc_registers()
        self._skip_dummy_frame()
        logger.info(f"[AD7606B {VERSION}] Ready (+-{self.v_range}V, SPS={self.sample_rate}, OSR=0x{self.osr:02X}, SPI={self.spi_speed/1000:.0f}kHz Mode2)")
    ...
    def _configure_adc_registers(self):
        """
        Регистры AD7606B:
        0x02 — CONFIGURATION (4-CH mode)
        0x03 — CHANNEL (sequencer off)
        0x04 — INPUT_RANGE:
               bits [1:0] = 00 → CH1-CH4: ±2.5V
               Итого: 0x00
        0x08 — OVERSAMPLING
        0x00 — CONFIGURATION (read/write)
        """
        self._reg_write(0x02, 0x08)
        self._reg_write(0x03, 0x00)
        self._reg_write(0x04, 0x00)   # CH1-CH4 = ±2.5V
    ...
    def _decode_raw_fast(self, raw_bytes):
        """
        Декодирует 8 байт (4 канала × 16 bit).
        Все каналы: ±2.5V (регистр 0x04 = 0x00).
        """
        raw_codes = np.frombuffer(raw_bytes, dtype='>i2')
        scales = np.array([
            2.5 / 32768.0,   # CH1: ±2.5V
            2.5 / 32768.0,   # CH2: ±2.5V
            2.5 / 32768.0,   # CH3: ±2.5V
            2.5 / 32768.0,   # CH4: ±2.5V (датчик воды)
        ], dtype=np.float64)
    ...
    def close(self):
        ...
        logger.info(f"[AD7606B {VERSION}] Stopped.")
