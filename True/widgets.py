...
from PyQt5.QtGui import QPainter, QPen, QColor, QRadialGradient, QPolygon, QFont, QPixmap, QCursor
...
class MapWidget(QWidget):
    ...
    def mouseReleaseEvent(self, event):
        if event.button() == Qt.LeftButton:
            if self.dragging:
                ...
                self._save_position()
                self.stationMoved.emit(self.station_offset_x, self.station_offset_y)
    ...
    def add_event(self, data):
        ...
        Возвращает True если принято, False если отфильтровано.
        """
        peak_mv = data.get('peak_amplitude_mv', 0)
        if peak_mv < EVENT_THRESHOLD_MV:
            logger.debug(
                f"[MAP] Событие отфильтровано: peak={peak_mv:.2f}mV < threshold={EVENT_THRESHOLD_MV}mV"
            )
            return False
    ...
    def _draw_event_labels(self, painter, events, cx, cy, px_per_km):
        """Рисует labels с выносками, избегая перекрытий."""
        placed_rects = []
        ...
        for evt in sorted_events:
            ...
            bg = QColor(center_color)
            bg.setAlpha(int(200 * opacity))
            ...
            tx = best_rect.x() + 3
            ty = best_rect.y() + line_h
            painter.drawText(tx, ty, label1)
            painter.drawText(tx, ty + line_h, label2)

            track_color = QColor(center_color)
            track_color.setAlpha(int(60 * opacity))
    ...
    def paintEvent(self, event):
        ...
        painter.drawText(cx - 4, 14, "N")
        painter.drawText(cx - 4, h - 4, "S")
        painter.drawText(w - 12, cy + 4, "E")
        painter.drawText(4, cy + 4, "W")
        ...
        for evt in grouped_events:
            ...
            for i in range(3):
                ...
                if pulse_r is not None and pulse_r > 0:
                    pulse_px = pulse_r * px_per_km
                    pulse_px = min(pulse_px, 60.0)
                    if pulse_px < 2:
                        continue
        ...
        for evt in grouped_arrows:
            ...
            start_x = cx + start_r * np.sin(rad)
            start_y = cy - start_r * np.cos(rad)
            end_x = cx + end_r * np.sin(rad)
            end_y = cy - end_r * np.cos(rad)
            if evt.event_type == 'explosion':
                arrow_color = QColor(255, 220, 0, arrow_alpha)
                type_letter = 'X'
            elif evt.event_type == 'earthquake':
                arrow_color = QColor(220, 20, 60, arrow_alpha)
                type_letter = 'M'
            else:
                arrow_color = QColor(100, 180, 255, arrow_alpha)
                type_letter = '?'
            ...
            dx = np.sin(rad)
            dy = -np.cos(rad)
            back_x = end_x - dx * arrow_size
            back_y = end_y - dy * arrow_size
            perp_x = -dy
            perp_y = dx
            ...
            if position_r > r_max * 0.9:
                ...
                fm = painter.fontMetrics()
                lbl1 = f"{type_letter}{evt.magnitude:.2f}"
...
class OscilloscopeWidget(pg.PlotWidget):
    """
    v9.2.2 — Carousel fix: set_data автоматически определяет формат данных.
    Скаляры → single mode. Кортежи (t,x,y,z) → dual mode с загрузкой.
    """
    ...
    def __init__(self, title="Channel"):
        ...
        self.dual_p = deque(maxlen=SPS)
        self.dual_z = deque(maxlen=SPS)

        pen_p = pg.mkPen(color='#00ff64', width=1.2)
        pen_z = pg.mkPen(color='#00c8ff', width=1.2)

        self.curve_p = pg.PlotDataItem(
            pen=pen_p, name='P-horiz',
            downsample=DS, clipToView=True
        )
    ...
    def set_data(self, data_list):
        ...
        - Пустой список → очистка текущего режима.
        - Скаляры → single mode.
        - Кортежи/списки (t, x, y, z) → dual mode.
        """
        if not data_list:
            # Очистка
            if self.mode == 'single':
            ...
            for item in data_list:
                if len(item) >= 4:
                    try:
                        ...
                        self.dual_p.append(np.sqrt(x*x + y*y))
    ...
    def update_data(self, t, x, y, z):
        ...
        p_amp = np.sqrt(float(x)**2 + float(y)**2)
        z_amp = float(z)

        self.dual_t.append(float(t))
        self.dual_p.append(p_amp)
        self.dual_z.append(z_amp)
    ...
    def _refresh_dual(self):
        ...
        p_display = p_arr - offset
        z_display = z_arr - offset

        self.curve_p.setData(x_arr, p_display)
        self.curve_z.setData(x_arr, z_display)

        self._last_ymax = ymax
        self.plotItem.setYRange(-ymax * 1.8, ymax * 1.0, padding=0)
