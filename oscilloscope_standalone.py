#!/usr/bin/env python3
"""
oscilloscope_standalone.py v7.1
...
class StandaloneOscilloscope(QMainWindow):
    def __init__(self):
        ...
        if config.GUI_CPU_CORES:
            try:
                ...
                logger.info(f"[OSC v7.1] GUI CPU affinity set to {config.GUI_CPU_CORES}")
            except Exception as e:
                logger.warning(f"[OSC v7.1] GUI CPU affinity failed: {e}")
        ...
        logger.info("[OSC v7.1] Heavy worker process started")

        # --- Processor ---
        self.processor = SeismicProcessor(event_queue=self.event_queue)

        # --- Acquisition process (v9.4.0) ---
        ...
        logger.info("[OSC v7.1] Acquisition process started")
        ...
        logger.info(f"[OSC v7.1] Started | SPS={SAMPLE_RATE} | GUI FPS=25 | BATCH=20")

    def load_config(self):
        try:
            with open(CONFIG_FILE, 'r', encoding='utf-8') as f:
                self.config = json.load(f)
            logger.info(f"[OSC v7.1] Config loaded: {len(self.config['channel_configs'])} channels")
        except Exception as e:
            logger.warning(f"[OSC v7.1] JSON ERROR: {e}, using defaults")
            self.config = {
                "x_seconds": 1.0,
                "channel_configs": [
    ...
    def closeEvent(self, event):
        logger.info("[OSC v7.1] Stopping acquisition, worker, timers...")
        self.stop_event.set()
        self.acquisition.join(timeout=3.0)
        if self.acquisition.is_alive():
            self.acquisition.terminate()
            logger.warning("[OSC v7.1] Acquisition process terminated forcefully")
        self.event_queue.put('STOP')
        self.heavy_process.join(timeout=3.0)
        if self.heavy_process.is_alive():
            self.heavy_process.terminate()
            logger.warning("[OSC v7.1] Heavy worker terminated forcefully")
        self.plot_timer.stop()
        self.water_timer.stop()
        self.result_timer.stop()
        self.daq_timer.stop()
        event.accept()
        logger.info("[OSC v7.1] UI closed cleanly.")
